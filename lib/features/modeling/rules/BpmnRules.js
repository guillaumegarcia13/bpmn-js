...
BpmnRules.prototype.init = function() {
  ...
  this.addRule([ 'shape.create', 'shape.append' ], function(context) {
    ...
    return canAttach([ shape ], target, position) || canCreate(shape, target, source, position);
  });
...
function includes(coll, elem) {
  return coll.indexOf(elem) !== -1;
}

function canAttach(elements, target, position) {
  var element, idx;
  ...
  switch (elements.length) {
    ...
    case 2:
      ...
      forEach(elements, function(elem) {
        if (is(elem, 'bpmn:BoundaryEvent')) {
          return results.push(elem.type);
        }
      ...
      if (!(includes(results, 'bpmn:BoundaryEvent') && includes(results, 'label'))) {
        return false;
      }

      idx = results.indexOf('bpmn:BoundaryEvent');
  ...
  if (((is(element, 'bpmn:IntermediateThrowEvent') && !element.parent) ||
    is(element, 'bpmn:BoundaryEvent')) &&
...
function canMove(elements, target) {
  ...
  forEach(attachers, function(attacher) {
    var hostIdx = rulesElements.indexOf(attacher.host),
        attacherIdx;

    if (hostIdx !== -1) {
      attacherIdx = rulesElements.indexOf(attacher);

      rulesElements.splice(attacherIdx, 1);
    }
  });

  return rulesElements.every(function(element) {
